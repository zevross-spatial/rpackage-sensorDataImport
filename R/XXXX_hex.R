...
hex_extract_convert <- function(thefile){
  ...
  startdate <- as.POSIXct(gsub('T' , '', info$start_date), origin = "1970-01-01")
...
createDateTime <- function(.data, startdate, seconds_column){
...
avgToSecond <- function(.data){
  ...
  .data <- mutate(.data, timesecond = floor(a)) %>% 
    group_by(timesecond) %>% summarise(avg = mean(b))
